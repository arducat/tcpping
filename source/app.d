		...
		stdout.flush();
                ...
                switch (input[0]) {
					...
					stdout.flush();
                                ...
                                break;
                        case "help":
                                cwritefln("<b>Команды:\n  connect [адрес:порт]</b> - Подключает к TCP серверу. Если адрес сервера не указан, Вас его спросят. <b><red>ИСПОЛЬЗОВАТЬ ДО \"send\"!</red>\n  send [сообщение]</b> - Отправляет сообщение на подключенный TCP сервер. <b><red>ИСПОЛЬЗОВАТЬ ПОСЛЕ \"connect\"!</red>\n  ver</b> - Выводит версию Консоли TcpPing.\n  <b>exit</b> - Выходит из Консоли TcpPing.");
                                break;
                        case "ver":
                                cwritefln("<b>Консоль TcpPing</b>:<b><grey> v0.1.3</grey></b>");
                                break;
